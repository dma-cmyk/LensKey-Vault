...
- **🧬 高度な生体認証 (WebAuthn PRF)**: WebAuthn PRF 拡張を使用。平文のパスワードを保存せず、生体認証成功時にのみデバイス内で生成される鍵を使用してデータを復号します。
- **🔒 オートロック機能**: 2分間の無操作、または南京錠アイコンのクリックによりボルトを即座に保護。
- **📱 QRコード・バックアップ**: 個別のアイテムを暗号化された状態のままQRコードとして出力。物理的なバックアップやデバイス間移行が安全に行えます。
- **📦 暗号化一括バックアップ**: すべてのデータをまとめてJSONファイルとしてエクスポート可能。バックアップファイル全体をパスワードで暗号化できるため、アイテム名などのメタデータも保護されます。
...
- **生体認証保護**: PRF (Pseudo-Random Function) 拡張により、ストレージ上から平文のパスワードを完全に排除。
- **メモリ・セキュリティ**: 使用後の機密バッファを自動的にゼロクリア（Sanitization）し、メモリダンプ攻撃を防御。
